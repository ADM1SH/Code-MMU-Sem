...
void View_Booked_Flights()
{
    ...
    while (fscanf(peakaboo, "%s %s %s %s %d %d %f %d %d %d %s %s %f",
                  passdata.Pass_Flight_code, passdata.Pass_Destinations, passdata.Pass_Flight_Class,
                  passdata.Ticket_Type, &passdata.Pass_Flight_NOWSeats, &passdata.Pass_Flight_MAXseats,
...
void CancelBooking() {
    FILE *original = fopen("flight_bookings.txt", "r");
    if (!original) {
    ...
    printf("Enter the full name to cancel booking: ");
    fgets(name_to_cancel, sizeof(name_to_cancel), stdin);

    char line[256];
    while (fgets(line, sizeof(line), original)) {
        if (!strstr(line, name_to_cancel)) { // Assuming each booking is a single line
            fputs(line, temp);
        }
    }
    ...
    rename("temp.txt", "flight_bookings.txt");
}
...
void Cancel_Booked_Flight() {
    ...
    while (1) {
        display_menu();
        choice = read_valid_integer("Enter your choice: ");

        switch (choice) {
            case 1:
                choice = Terminal_options(); // Corrected: Assign return value
                break;
            case 2:
                Book_Flight(); // Corrected function name
                break;
            case 3:
                View_Booked_Flights(); // Corrected function name
                break;
            case 4:
